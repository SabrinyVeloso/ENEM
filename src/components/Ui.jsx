...
export function GlassCard({ className = '', children }) {
  return (
    <div className={`ui-card rounded-[28px] border border-[var(--border)] bg-[rgba(255,255,255,0.08)] shadow-[0_20px_70px_rgba(0,0,0,0.12)] backdrop-blur-2xl ${className}`}>
      {children}
    </div>
  );
}
      ...
      <div className="space-y-1">
        {(eyebrow || icon) ? (
          <div className="flex items-center gap-2">
            {icon ? (
              <span className="grid h-8 w-8 place-items-center rounded-2xl border border-[var(--border)] bg-white/10 text-[var(--accent)] shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
                {icon}
              </span>
            ) : null}
            {eyebrow ? <p className="text-[11px] font-extrabold uppercase tracking-[0.28em] text-[var(--accent)]">{eyebrow}</p> : null}
          </div>
        ) : null}
...
export function FilterChip({ active, children, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`min-h-11 rounded-full px-4 py-2 text-sm font-extrabold transition active:scale-[0.98] ${
        active
          ? 'bg-[linear-gradient(135deg,var(--primary),var(--accent))] text-white shadow-lg shadow-[var(--glow)]'
          : 'border border-[var(--border)] bg-[rgba(255,255,255,0.1)] text-[var(--text)]'
      }`}
    >
      {children}
    </button>
...
export function StatusBadge({ status, children }) {
  ...
  return <span className={`inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-black uppercase tracking-[0.22em] ${palette[status] || palette.pending}`}>{children}</span>;
}
