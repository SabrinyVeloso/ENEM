    ...
    <div className="grid gap-4 pb-6">
      ...
      <section className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <GlassCard className="p-4 sm:p-5">
          <SectionHeader eyebrow="Progresso" title="Estatísticas da revisão" subtitle="Esses números são atualizados conforme você responde os cards." />
          <div className="mt-4 grid gap-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-[20px] border border-[var(--border)] bg-white/5 p-4">
                <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[var(--muted)]">Últimos 7 dias</p>
                <strong className="mt-1 block text-2xl font-black text-[var(--text)]">{dashboard.flashcardWeeklyHistory.length}</strong>
              </div>
              <div className="rounded-[20px] border border-[var(--border)] bg-white/5 p-4">
                <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[var(--muted)]">Este mês</p>
                <strong className="mt-1 block text-2xl font-black text-[var(--text)]">{dashboard.flashcardMonthlyHistory.length}</strong>
              </div>
            </div>
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-4 sm:p-5">
          <SectionHeader eyebrow="Matérias" title="Cobertura dos flashcards" subtitle="Veja a distribuição da fila antes de iniciar a sessão." />
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {subjectEntries.length === 0 ? (
              <EmptyState title="Ainda sem dados" subtitle="Conclua conteúdos do cronograma para gerar cartões de revisão." />
            ) : (
              subjectEntries.map(([subject, count]) => (
                <SubjectSummary key={subject} label={subjectLabels[subject] || subject} count={count} total={totalCards} />
              ))
            )}
          </div>
        </GlassCard>
      ...
      <GlassCard className="p-4 sm:p-5">
        <SectionHeader eyebrow="Atenção" title="Cards que merecem reforço" subtitle="Itens com mais erros aparecem como prioridade nas próximas sessões." />
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {difficultCards.length === 0 ? (
            <EmptyState title="Nenhum ponto crítico" subtitle="Quando algum card acumular erros, ele aparecerá aqui para orientar sua revisão." />
          ) : (
              ...
              <div key={card.id} className="rounded-[20px] border border-[var(--border)] bg-white/5 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <strong className="mt-1 block text-sm font-black text-[var(--text)]">{card.sourceTitle}</strong>
                  </div>
                  <StatusBadge status="perdido">{card.incorrect} erros</StatusBadge>
                </div>
                <p className="mt-3 text-sm leading-6 text-[var(--muted)]">{card.front}</p>
              </div>
            ))
          )}
        </div>
