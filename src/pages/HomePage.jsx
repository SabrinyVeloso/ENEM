...
export default function HomePage() {
  ...
  const todayItems = dashboard.todayReviewDay
    ? dashboard.todayReviewItems.slice(0, 4)
    : dashboard.todayStudyItems.slice(0, 4);
  const motivation = dashboard.motivations[0];
  const hasMoreToday = dashboard.todayReviewDay
    ? dashboard.todayReviewItems.length > todayItems.length
    : dashboard.todayStudyItems.length > todayItems.length;
  const nextActiveDay = schedule.weeks
    .flatMap((week) => week.days)
    .find(
      (day) =>
        day.date > today && (day.type === "study" || day.type === "review"),
    );
  ...
  const overduePendingItems = schedule.weeks
    ...
    .filter(
      (day) =>
        ...
        day.date < today &&
        day.contentId &&
        getContentStatus(day.contentId) === "pending",
    )
  ...
  const overdueItems = schedule.weeks
    ...
    .filter(
      (day) =>
        ...
        day.date < today &&
        day.contentId &&
        getContentStatus(day.contentId) === "perdido",
    )
    .map((day) => day)
    .filter(Boolean)
    .slice(0, 10);
    ...
    <div className="grid gap-4 pb-6">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        ...
        <GlassCard className="p-4 sm:p-5">
          <SectionHeader
            eyebrow={
              dashboard.todayReviewDay ? "Revisão de hoje" : "Estudos de hoje"
            }
            title={
          ...
          <div className="mt-4 grid gap-3">
            {todayItems.length === 0 ? (
              <EmptyState
                title={
                  dashboard.todayReviewDay
                    ? "Sem flashcards para hoje"
                    : "Sem estudos para hoje"
                }
                subtitle={
                  dashboard.todayReviewDay
                    ? "Ainda não há cartões liberados para a revisão de hoje."
                    : "Seu cronograma não tem novos itens programados para hoje."
                }
              />
            ) : dashboard.todayReviewDay ? (
              <div className="rounded-[24px] border border-[var(--border)] bg-gradient-to-br from-blue-500/10 to-purple-500/10 p-4">
                <p className="text-sm font-semibold">
                  ...
                  <span className="font-black">
                    {dashboard.todayReviewItems.length} flashcards
                  </span>{" "}
                  para revisar hoje.
                </p>
                <button
              ...
              todayItems.map((item) => (
                <StudyItemCard
                  key={item.id}
                  item={item}
                  onStatus={actions.setContentStatus}
                />
              ))
            )}
          </div>
          {!dashboard.todayReviewDay && hasMoreToday ? (
            <p className="mt-3 text-sm font-semibold text-[var(--muted)]">
              + {dashboard.todayStudyItems.length - todayItems.length} itens
              também estão programados para hoje.
            </p>
          ) : null}
        </GlassCard>
      </section>
